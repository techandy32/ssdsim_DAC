...
int Region0_Block[2] = {-1, -1};
int Region1_Block[2] = {-1, -1};
int Region2_Block[2] = {-1, -1};
...
struct ssd_info *pre_process_page(struct ssd_info *ssd)
{
    ...
    {
        sscanf(buffer_request,"%ld %d %d %d %d",&time,&device,&lsn,&size,&ope);
        fl++;
...
unsigned int get_new_block_LRU(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane){
    ...
    for(i=0;i<ssd->parameter->block_plane;i++){
        ...
        if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].last_write_page < 8){
            block = i;
            #ifdef MPDEBUG 
    ...
    if(block == -1){
    	for(i=0;i<ssd->parameter->block_plane;i++){
            printf("%d last page %d\n",i ,ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].last_write_page);
	}
        printf("get_new_block_LRU Error\n");
        while(1){};
    }
    return block;
} 
...
{
    unsigned int i, Hotness;
    struct local *new_location=NULL;
    unsigned int old_active_block = 0;
    ...
    Hotness = Hotness_DAC(ssd, location);

    old_active_block = ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block;

    switch(Hotness){
        case 0:
            #ifdef MPDEBUG
            printf("Region0\n");
            printf("block num : %d\n",Region0_Block[location->plane]);
            printf("free page : %d\n",ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Region0_Block[location->plane]].free_page_num );
            #endif
            if( Region0_Block[location->plane] == -1 || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Region0_Block[location->plane]].free_page_num == 0)
            {
                //*(Hot_Block+location->plane) = Hot_Block[location->plane] (hotblock[2]={69,420})
                Region0_Block[location->plane] = get_new_block_LRU(ssd, location -> channel, location -> chip, location -> die, location -> plane);
            }
            if(Region0_Block[location->plane] != -1){
                ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = Region0_Block[location->plane];
            }
        ...
        case 1:
            ...
            printf("Region1\n");
            #endif
            if(Region1_Block[location->plane] == -1 || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Region1_Block[location->plane]].free_page_num == 0)
            {
                //*(Hot_Block+location->plane) = Hot_Block[location->plane] (hotblock[2]={69,420})
                Region1_Block[location->plane] = get_new_block_LRU(ssd, location -> channel, location -> chip, location -> die, location -> plane);
            }
            if(Region1_Block[location->plane] != -1){
                ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = Region1_Block[location->plane];
            }
            ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
            break;
        ...
        case 2:
            ...
            printf("Region2\n");
            #endif
            if(Region2_Block[location->plane] == -1 || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[Region2_Block[location->plane]].free_page_num == 0)
            {
                //*(Hot_Block+location->plane) = Hot_Block[location->plane] (hotblock[2]={69,420})
                Region2_Block[location->plane] = get_new_block_LRU(ssd, location -> channel, location -> chip, location -> die, location -> plane);
            }
            if(Region2_Block[location->plane] != -1){
                ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = Region2_Block[location->plane];
            }
            ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
            break;
    }
    ...
    // *note : must return old active block
    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = old_active_block;
